...
function LazyThumb({ fileId, pageNo, width, style }) {
...
// ---------- Left sidebar ----------
function Sidebar({ t, lang, files, onAdd, onSample, onRemove, onMoveFile, recent, stage, progress, acceptImages, allowReorder }) {
  const inputRef = React.useRef(null);
  return (
    <aside style={{
      width: 248, flex: "none", borderRight: "1px solid var(--border-default)", background: "var(--surface-card)",
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 14, display: "flex", flexDirection: "column", gap: 18 }}>
        <section style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            }}>{window.WSIcons.plus(13)}{t.sidebar.addFile}</button>
          ...
          {files.length === 0 && (
            ...
            }}>{window.WSIcons.sparkle(14)}{t.sidebar.sample}</button>
          )}
              ...
              <div key={f.id} style={{
                ...
                <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 2 }}>
                  ...
                  <span style={{ font: "10.5px/1 var(--font-sans)", color: "var(--text-muted)" }}>
                    {window.PdfEngine.fmtSize(f.size, lang)} · {f.pageCount} {t.success.pages}
                  </span>
                </div>
        ...
        {stage === "processing" && (
          <section style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            <VDS.ProgressBar value={progress} />
          </section>
        )}
      ...
      <div style={{ padding: 14, borderTop: "1px solid var(--border-default)", display: "flex", flexDirection: "column", gap: 6 }}>
...
function PageGrid({ t, pages, selection, setSelection, cardWidth = 148, onReorder, onRotate, onDelete, onDuplicate, selectable = true, badges }) {
  ...
  return (
    <div style={{ flex: 1, overflow: "auto", padding: "20px 24px" }}
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 16, alignContent: "flex-start" }} role="listbox" aria-multiselectable="true">
        {live.map((p, i) => (
          <div key={p.uid}
            draggable={!!onReorder}
            ...
            <VDS.PageCard
              ...
              rotation={p.rotation}
              badge={badges ? badges(p, i) : (p.rotation ? <VDS.Badge tone="brand">{p.rotation}°</VDS.Badge> : null)}
      ...
      {menu && (onRotate || onDelete || onDuplicate) && (
        <VDS.ContextMenu x={menu.x} y={menu.y} onClose={() => setMenu(null)} items={[
          onRotate && { label: t.pageMenu.rotateR, shortcut: "R", icon: window.WSIcons.rotate(15), onSelect: () => onRotate([menu.uid], 90) },
          onRotate && { label: t.pageMenu.rotateL, icon: window.WSIcons.rotate(15), onSelect: () => onRotate([menu.uid], -90) },
...
function DocPreview({ t, pages, overlay, children }) {
  ...
  return (
    <div style={{ flex: 1, position: "relative", display: "flex", flexDirection: "column", minWidth: 0 }}>
      <div ref={scroller} onScroll={onScroll} style={{ flex: 1, overflow: "auto", padding: "24px 24px 90px", display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
        {live.map((p, i) => (
      ...
      <div style={{ position: "absolute", bottom: 18, left: "50%", transform: "translateX(-50%)", zIndex: 10 }}>
        <VDS.Toolbar>
          <VDS.ZoomControl value={zoom} onZoomIn={() => setZoom((z) => Math.min(220, z + 20))} onZoomOut={() => setZoom((z) => Math.max(40, z - 20))} onReset={() => setZoom(100)} />
          <VDS.ToolbarDivider></VDS.ToolbarDivider>
          <VDS.PageNavigator page={page} count={live.length} onChange={goTo} />
          {children}
        </VDS.Toolbar>
      </div>
    </div>
...
Object.assign(window, { LazyThumb, Sidebar, PageGrid, DocPreview });
