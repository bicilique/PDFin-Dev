...
const { Field: F3, Segmented: Seg3, SliderRow: SR3, TX: TX3, TOOL_DEFS: DEFS3 } = window;
  ...
  Panel: ({ t, lang, opts, setOpts }) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <T3DS.Input type="password" label={TX3(lang, "Kata sandi", "Password")} value={opts.pw} onChange={(e) => setOpts({ ...opts, pw: e.target.value })} />
        <StrengthMeter pw={opts.pw} lang={lang} />
      </div>
      <T3DS.Input type="password" label={TX3(lang, "Ulangi kata sandi", "Repeat password")} value={opts.pw2}
      ...
      <F3 label={TX3(lang, "Izin dokumen", "Document permissions")}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <T3DS.Switch label={TX3(lang, "Boleh dicetak", "Allow printing")} checked={opts.printing} onChange={(printing) => setOpts({ ...opts, printing })} />
          <T3DS.Switch label={TX3(lang, "Boleh disalin", "Allow copying")} checked={opts.copying} onChange={(copying) => setOpts({ ...opts, copying })} />
          <T3DS.Switch label={TX3(lang, "Boleh diubah", "Allow editing")} checked={opts.editing} onChange={(editing) => setOpts({ ...opts, editing })} />
        </div>
      </F3>
...
DEFS3.unlock = {
  ...
  Panel: ({ t, lang, opts, setOpts }) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <T3DS.Input type="password" label={TX3(lang, "Kata sandi dokumen", "Document password")} value={opts.pw}
      ...
      <T3DS.Alert tone="info">{TX3(lang,
        "PDFin membuka kunci file yang kata sandinya Anda ketahui — bukan alat pembobol.",
        "PDFin unlocks files whose password you already know — it is not a cracking tool.")}</T3DS.Alert>
    </div>
  ),
  ...
  Panel: ({ t, lang, opts, setOpts, ctx }) => {
    ...
    React.useEffect(() => {
      if (fileId && opts.loadedFor !== fileId) {
        window.PdfProcess.readMetadata(fileId).then((m) => setOpts({ ...opts, ...m, loadedFor: fileId }));
      }
    }, [fileId]);
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <T3DS.Input label={TX3(lang, "Judul", "Title")} value={opts.title} onChange={(e) => setOpts({ ...opts, title: e.target.value })} />
        <T3DS.Input label={TX3(lang, "Penulis", "Author")} value={opts.author} onChange={(e) => setOpts({ ...opts, author: e.target.value })} />
        <T3DS.Input label={TX3(lang, "Subjek", "Subject")} value={opts.subject} onChange={(e) => setOpts({ ...opts, subject: e.target.value })} />
        <T3DS.Input label={TX3(lang, "Kata kunci", "Keywords")} value={opts.keywords} hint={TX3(lang, "Pisahkan dengan koma.", "Separate with commas.")} onChange={(e) => setOpts({ ...opts, keywords: e.target.value })} />
      </div>
    );
  },
...
function SignPad({ lang, onChange }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {tab === "type" ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <T3DS.Input label={TX3(lang, "Nama", "Name")} value={name} onChange={(e) => typeName(e.target.value)} />
          {name.trim() && (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          ...
          <T3DS.Button variant="ghost" size="sm" onClick={clear}>{TX3(lang, "Bersihkan", "Clear")}</T3DS.Button>
        </div>
      )}
    </div>
  );
...
DEFS3.sign = {
  ...
  Panel: ({ t, lang, opts, setOpts }) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <T3DS.Alert tone={opts.pageIndex == null ? "warning" : "info"}>
        {opts.pageIndex == null
...
DEFS3.ocr = {
  ...
  Panel: ({ t, lang, opts, setOpts, ctx }) => (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <T3DS.Select label={TX3(lang, "Bahasa dokumen", "Document language")} value={opts.language} onChange={(e) => setOpts({ ...opts, language: e.target.value })}
        options={[
      ...
      <T3DS.Alert tone="info">{TX3(lang,
        ...
        "Scanned text is recognized and embedded as a searchable layer — the page appearance does not change.")}</T3DS.Alert>
    </div>
  ),
